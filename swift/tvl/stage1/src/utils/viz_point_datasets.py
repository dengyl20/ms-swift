...
Only 3 views are rendered: front / top / bottom (正视/俯视/仰视).
...
# Common
POINTNUM: int = 8192
OUTPUT_DIR: str = "/vast/users/guangyi.chen/causal_group/yunlong.deng/Multimodal/ms-swift/swift/point_cloud/stage1/src/tmp/pointcloud_renders"
RANDOM_SEED: int = 0
...
# - 俯视图: elev=90          (camera on +Z looking down)
# - 仰视图: elev=-90         (camera on -Z looking up)
...
DATASET1_JSONL_PATH: str = ""  # <<< fill in: path/to/dataset1_results.jsonl
...
# JSONL that drives what to render (NEW)
DATASET2_JSONL_PATH: str = ""  # <<< fill in: path/to/dataset2_results.jsonl
...
DATASET2_COLOR_MODE: str = "last3"
DATASET2_CONSTANT_RGB: Tuple[float, float, float] = (0.75, 0.75, 0.75)
...
def main() -> None:
    ...
    print("Manifests saved as manifest.json in each folder.")
    print("=" * 80)
